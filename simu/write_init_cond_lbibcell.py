...
def calculate_center(size_x, size_y, radius):
    ...
    if (size_x - x <= radius) or (size_y - y <= radius):
        raise ValueError(
            "! Radius {} is too large for grid ({}, {})".format(radius, size_x, size_y)
        )
...
if __name__ == "__main__":
    ...
    filename = "{}/parameters_{}_by_{}_radius_{}_center_res_{}.txt".format(
        out_dir, size_x, size_y, int(radius), res
    )
