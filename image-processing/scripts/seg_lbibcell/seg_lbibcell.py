...
from skimage import (
    color, filters, measure, morphology, segmentation, io
)
...
def seg_lbibcell(filename, outdir, size_x, size_y, show=False):
    ...
    ax[1].imshow(segmentation.mark_boundaries(
        img, labeled_cells_threshold_multiotsu))
    ax[1].set_title(
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        prog='seg_lbibcell',
        description="Segementate image of cell after LBIBCell"
    )
    parser.add_argument(
        "-i", "--input", type=str, required=True, help="Image saved from Paraview, i.e. screenshot.png"
    )
    parser.add_argument(
        "--outdir", type=str, help="Diretory to create plots and save statistics about the cell"
    )
    parser.add_argument(
        "-x", "--SizeX", type=int, default=1000, help="The X-axis for the box grid, i.e. 1000, by default 1000"
    )
    parser.add_argument(
        "-y", "--SizeY", type=int, default=1000, help="The Y-axis for the box grid, i.e. 1000, by default 1000"
    )
    parser.add_argument(
